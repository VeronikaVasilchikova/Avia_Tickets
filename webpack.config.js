const path = require('path'); // path - inbuild module from Node.js
const autoprefixer = require('autoprefixer'); //installed module for vendor CSS prefixes
...
module.exports = {
  ...
  entry: {
    // polyfill: 'babel-polyfill', // for new ES6 features emulation
  ...
  devServer: { // локальный сервер
    ...
    noInfo: false,
    stats: 'minimal',
    hot: true, // перезагрузка сразу после изменений
  },
  module: {
    ...
    rules: [
      {
        ...
        test: /\.js$/, // all files .js will be passed through babel
      },
      {
        test: /\.css$/,
        use: [
